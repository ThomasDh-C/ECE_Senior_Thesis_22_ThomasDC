...
def test_conv2d_unpadded():
    ...
    y = relay.Var("y", type_annotation=relay.TensorType(
        (k_num, k_chan, k_row, k_col)))  # 3,  3px,3px  3ch
    ...
    inp = np.random.uniform(-1, 1, (1, in_chan, in_row,
                            in_col)).astype("float32")
    # weight is kernel matrix to drag across
    wgt = np.random.uniform(-1, 1, (k_num, k_chan,
                            k_row, k_col)).astype("float32")

    # why is this needed - didn't we just make in this shape
